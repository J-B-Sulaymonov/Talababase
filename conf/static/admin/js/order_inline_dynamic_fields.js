function toggleFieldsForInline(inlineRow) {
    ...
    // YANGI FIELD: TSCH kim tomonidan
    const tschByWhomField = inlineRow.querySelector(".field-tsch_by_whom");
    ...
    function toggle() {
        ...
        // 🔸 YANGI: TSCH kim tomonidan — faqat safidan chiqarishda
        if (tschByWhomField) {
            // Select yoki input bo'lishi mumkin (choices bo'lgani uchun select bo'ladi)
            const input = tschByWhomField.querySelector("input, select");
            tschByWhomField.style.display = isExpulsion ? "" : "none";
            if (input) {
                if (isExpulsion) {
                    input.setAttribute("required", "required");
                    updateRequiredMark(tschByWhomField, true);
                } else {
                    input.removeAttribute("required");
                    updateRequiredMark(tschByWhomField, false);
                }
            }
        }
        ...
        if (isAdmission) {
            [tschField, tschByWhomField, appDateField, docDateField].forEach((f) => {
                if (!f) return;
...
document.addEventListener("submit", function (e) {
    ...
    form.querySelectorAll(".dynamic-order_set, .inline-related").forEach((inlineRow) => {
        ...
        const tschByWhom = inlineRow.querySelector('[id$="-tsch_by_whom"]');
        const app = inlineRow.querySelector('[id$="-application_date"]');

        // Validatsiya: Ariza sanasi
        ...
        // Validatsiya: TSCH kim tomonidan (YANGI)
        if (text.includes("safidan chiqarish") && tschByWhom && !tschByWhom.value.trim()) {
            tschByWhom.reportValidity();
            tschByWhom.scrollIntoView({ behavior: "smooth", block: "center" });
            valid = false;
        }
    });
